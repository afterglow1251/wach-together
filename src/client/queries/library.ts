...
export function useUpdateLibraryStatus() {
  ...
  return useMutation(() => ({
    ...
    onError: (_err: unknown, _data: LibraryUpdateRequest, context: { prev: [unknown, LibraryResponse | undefined][] } | undefined) => {
      context?.prev.forEach(([key, data]) => qc.setQueryData(key as string[], data));
    },
    onSettled: () => qc.invalidateQueries({ queryKey: ["library"] }),
  }));
...
export function useRemoveFromLibrary() {
  ...
  return useMutation(() => ({
    ...
    onError: (_err: unknown, _data: LibraryDeleteRequest, context: { prev: [unknown, LibraryResponse | undefined][] } | undefined) => {
      context?.prev.forEach(([key, data]) => qc.setQueryData(key as string[], data));
    },
    onSettled: () => qc.invalidateQueries({ queryKey: ["library"] }),
  }));
