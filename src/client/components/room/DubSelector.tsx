    ...
    <Show when={props.dubs.length > 1}>
      <div class="px-5 py-4 sidebar-divider relative z-1">
        ...
        <select
          ...
          class="w-full px-3.5 py-2.5 bg-input border border-border rounded-md text-text text-sm outline-none cursor-pointer transition-colors focus:border-accent select-styled disabled:cursor-not-allowed disabled:opacity-70"
        >
          <For each={props.dubs}>{(dub, i) => <option value={i()}>{dub.name}</option>}</For>
        </select>
      </div>
    </Show>
  )
