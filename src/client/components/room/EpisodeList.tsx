            ...
            return (
              ...
              >
                <Show when={!isLoading()} fallback={<Spinner />}>
                  <span
                    class={`text-[11px] font-bold min-w-5 ${isActive() ? "text-accent" : isWatched() ? "opacity-50" : "text-muted"}`}
                  >
                    {i() + 1}
                  </span>
                </Show>
                <span
                  class={`flex-1 min-w-0 overflow-hidden text-ellipsis whitespace-nowrap ${isWatched() ? "opacity-50" : ""}`}
                >
                  {ep.name}
                </span>
                <button
